...
class _OrderConfirmationCartPageState extends State<OrderConfirmationCartPage>
  ...
  bool get isPoin => widget.metodePembayaran == 'Poin';

  int _parseValue(String value) {
    if (value.isEmpty) return 0;

    if (isPoin) {
      // Untuk Poin: langsung parse string angka
      return int.tryParse(value) ?? 0;
    } else {
      // Untuk COD: bersihkan format Rupiah
            ...
            SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Product Items - Simplified Layout
                  _buildProductList(),
                          ...
                          isPoin
                              ? Row(
                                  children: [
                                    ...
                                    Text(
                                      numberFormat
                                          .format(_parseValue(widget.ongkir)),
                                      style: const TextStyle(
                                        fontSize: 14,
                                        fontWeight: FontWeight.w600,
                                        color: Color(0xFF1A1A1A),
  ...
  Widget _buildProductList() {
    return Column(
      children: widget.namaProduk.asMap().entries.map((entry) {
        int index = entry.key;
            ...
            children: [
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        // Weight
                        Text(
                          "Total: ${widget.berat[index]} ${widget.satuan[index]}",
                          style: const TextStyle(
                            fontSize: 12,
                            color: Color(0xFF666666),
                            fontFamily: 'Poppins',
              ...
              Column(
                ...
                children: [
                  isPoin
                      ? Row(
                          children: [
                            ...
                            Text(
                              numberFormat.format(
                                  _parseValue(widget.hargaProduk[index])),
                              style: const TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.w600,
                                color: Color(0xFF589400),
                  ...
                  const SizedBox(height: 4),
                  Text(
                    "Total: ${isPoin ? numberFormat.format(_parseValue(widget.hargaProduk[index]) * widget.jumlah[index]) : _calculateTotalPrice(widget.hargaProduk[index], widget.jumlah[index])}",
                    style: const TextStyle(
                      fontSize: 11,
  ...
  String _calculateTotalPrice(String price, int quantity) {
    if (isPoin) {
      final total = _parseValue(price) * quantity;
      return numberFormat.format(total);
    } else {
      // Untuk Rupiah, bersihkan format dan hitung total
      final cleanPrice = price
          .replaceAll('Rp. ', '')
          .replaceAll('.', '')
          .replaceAll(',', '')
          .trim();
      final priceValue = int.tryParse(cleanPrice) ?? 0;
